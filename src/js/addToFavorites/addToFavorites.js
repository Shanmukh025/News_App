...
import { clear, renderMarkup } from '../renderMarkup';
      ...
      if (currentPage === 'Favorite') {
        ...
        if (updatedFavorites.length === 0) {
          refs.gallery.insertAdjacentHTML(
            'beforeend',
            "<h2 class='fav-not-found'>Favorites are empty!</h2>"
          );
        }
      }
    } else {
...
function createFavorite() {
  ...
  if (favorites.length === 0) {
    refs.gallery.insertAdjacentHTML(
      'beforeend',
      "<h2 class='fav-not-found'>You haven't added any Favorite Articles!</h2>"
    );
    return;
  }
